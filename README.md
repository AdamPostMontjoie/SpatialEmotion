...
- **Removal of `unchecked @Sendable`:** Originally, the `ARSession` was being passed directly to the TCA state so I could pass it into the LiDAR and Face dependencies that extract the `ARAnchor` data and return URLs linking to the `.usdz` files. This was dangerous behavior, increased the risk of the dependencies extracting the anchors at the wrong time, and also made the entire camera reducer untestable. By removing this and instead letting the `ARSessionDelegate` handle the dependencies and only giving state back the URLs, I fixed the reducer's testability. I realized after that refactoring the dependencies to accept just the anchors as an argument would increase testability and would not require the use of `unchecked @Sendable`, and so the dependencies were extracted back out again for separation of concerns.
 ...
 -**Benefits of TestStore:**   Writing tests helped me catch other improper usage of `@Sendable` in my code. Inside my Database dependency, I had been sending an array filled with objects with the type of a `@Model` class to the main thread. 
